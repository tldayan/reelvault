...
export default function ShowDetails({ showId, setShowEpisodeLink}) {
...
useEffect(() => {
    const seasonsContainerElement = document.querySelector(".season_list_container");
...
const handleEpisodeSelect = (seasonNumber, episodeNumber) => {
    setShowEpisodeLink(`https://vidsrc.me/embed/tv?tmdb=${showId}&season=${seasonNumber}&episode=${episodeNumber}`)
  ...
  return (
    ...
    <div className='movie_details_container'>
      <img className='movie_details_poster' src={`https://image.tmdb.org/t/p/w154${showData.poster_path}`} alt="" />
      <div className='show_info_container'>
        ...
        <div className='movie_stats_container'>
          <div className='first_stats_container'>
            {genres.length > 0 && (
            <p>Genre: {genres.map((eachgenre) => eachgenre.name).join(', ')}</p>
            )}
            <p>Language: {showData.original_language && showData.original_language.toUpperCase()}</p>
          </div>
          <div className='second_stats_container'>
            ...
            {productionCompanies.length > 0 && (
              <p>Production: {productionCompanies.map((eachCompany) => eachCompany.name).join(', ')}</p>
            )}
          </div>
        </div>
      </div>
      <div className="shows_list_container">
        <div className="main_season_list_container">
            <button onClick={openSeasonsContainer} className='seasons_button'>Season {selectedSeason} &#9660;</button>
            <ul className='season_list_container hide'>
                {seasonList.map((eachSeason,index) => {
                    return <li key={eachSeason.id}><button onClick={() => handleSeasonSelect(index)} className='season_button'>Season {index + 1}</button></li>
                })}
            </ul>
        </div>
        <div className="main_episode_list_container">
            <ul className='episode_list_container'>
                {episodeList.map((eachEpisode,index)=> {
                    return <li key={index}><button onClick={() => handleEpisodeSelect(selectedSeason,index + 1)} className={`episode_buttons ${selectedEpisode === (index + 1) ? "active" : null}`}>Episode {index + 1}</button></li>
                })}
            </ul>
        </div>
      </div>
