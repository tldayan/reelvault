...
export default function ShowCard({ eachShow }) {
  ...
  return (
    <Link to={`tvshows/${eachShow.id}`} className="movie_container" key={eachShow.id}>
      ...
      <h3 className='movie_name'>{eachShow.name.length > 20 ? `${eachShow.name.slice(0, 20)}...` : eachShow.name}</h3>
    </Link>
  );
}
