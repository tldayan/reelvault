        ...
        recommendedShowData.map(eachShow => { 
            if(eachShow?.poster_path && eachShow?.vote_count > 50 && eachShow?.first_air_date?.slice(0,4) > 1993) {
                ...
                <swiper-slide key={eachShow?.id}>
                    <Link className='recommended_link' onClick={() => handleNameChange(eachShow?.original_name)} to={`../tvshows/${eachShow?.id}`}><img className='recommended_movie_poster' src={eachShow?.poster_path === null ? defaultPoster : `https://image.tmdb.org/t/p/original${eachShow?.poster_path}`} alt="" /><p className='recommended_movie_title'>{eachShow?.name.length > 15  ? `${eachShow?.name.slice(0, 15)}...` : eachShow?.name}</p>
